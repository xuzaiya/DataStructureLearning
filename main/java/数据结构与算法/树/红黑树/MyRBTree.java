...
public class MyRBTree<T extends Comparable<T>,D> {
    ...
    * */
    ...
    public RBNode<T,D> search(T key,RBNode<T,D>node){
        if (node !=null){
            //查找的过程，就是一直递归比较叶子节点为止
            int com = key.compareTo(node.key);
            // com<0 说明key小于node.key
            if(com<0){
                return search(key,node.leftChild);
            }else if(com>0){
                // com>0 说明key大于node.key
 ...
 *   ly ry               lx ly
    对某个节点进行左旋
    当前节点就是父亲节点，整体过程就是父亲下沉，右孩子上升，然后右孩子的左节点变成原父亲节点的右节点
     */

    private void leftRotate2(RBNode<T,D>x){
        //1.x左旋, 将x的右节点赋值给y,将y的左子节点赋值给x的右节点
        RBNode<T,D> y = x.rightChild;
        x.rightChild = y.leftChild;
        //2.判断y的左子节点是不是为空，不为空，则父节点指向x
        if(y.leftChild!=null){
    ...
    public void leftRonate(RBNode<T,D>x){
        ...
        if(x.parent!=null){
            if (x.parent.leftChild == x){//如果x是左子节点
                x.parent.leftChild = y;
            }else{
                x.parent.rightChild = y;
            }
        }else{
            this.root = y;//如果x的父亲节点，则将y设置为根节点
        }
        x.parent = y;
    }
    ...
    /*
    对某节点进行右旋
    当前节点就是父亲节点，整体过程就是父亲下沉，左孩子上升，然后左孩子的右节点变成原父亲节点的左节点
 *        p                   p
 *       /                   /
 *      y                   x
 *     / \                 / \
 ...
 * 3. 将x的右子节点设为y，将y的父节点设为x
    ...
    public void rightRonate3(RBNode<T,D>y){
        //右旋
        RBNode<T,D> x = y.leftChild;
        ...
        //将x的右节点设置为y,将y的父节点设置为x
        y.parent = x;
        x.rightChild = y;

    }

    public void rightRonate(RBNode<T,D>y){
        RBNode<T,D> x = y.leftChild;
        if (x.rightChild!=null){
            x.rightChild.parent = y;
        }
    ...
    public void rightRonate2(RBNode<T,D>y){
        RBNode<T,D> x  = y.leftChild;
        if(x.rightChild!=null){
            x.rightChild.parent = y;
        }
     ...
     * @param node 新插入的节点
     */
    public void balanceInsertion2(RBNode<T,D>node){
        RBNode<T,D> parent,gparent;//定义父节点和祖父节点
        while((parent=parentOf(node))!=null && isRed(parent)){
            //父节点为不为空，并且是红的
            gparent = parentOf(parent);//拿到其祖父节点
            if(parent == gparent.leftChild){
                ...
                //case 2 : 叔叔节点是黑色的 当前节点是右子节点
                if(node == parent.rightChild){
                    leftRonate(parent);
                    RBNode<T,D>temp = parent;
                    parent = node;
                    node = temp;
                }
                ...
                //若父亲节点是祖父的右子节点，与上面情况完全相反，本质是一样的
